 ...
 */
export function detectScheduleConflicts(newEvent, existingEvents, staff, isUpdate = false) {
    const conflicts = [];
    ...
    if (staffMember) {
        ...
        if (!staffMember.availability[dayOfWeek]) {
            ...
            if (staffMember.blockedTimes) {
                ...
                const blockedTimesForDate = staffMember.blockedTimes.filter(bt => bt.date === newEventDate);
    ...
    if (newEvent.machineId) {
        const machineEvents = events.filter(event => event.machineId === newEvent.machineId);
 ...
 */
export function hasScheduleConflicts(event, existingEvents, staff, isUpdate = false) {
 ...
 */
export function getAvailableTimeSlots(staffId, date, staff, existingEvents, minimumDuration = 30) {
    ...
    const staffEvents = existingEvents.filter(event => event.staffId === staffId &&
