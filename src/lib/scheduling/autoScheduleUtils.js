...
function calculateSkillRelevance(staffMember, job) {
    ...
    let matchCount = staffMember.skills.filter(skill => skill.toLowerCase().includes(jobTypeString) ||
        jobTypeString.includes(skill.toLowerCase())).length;
 ...
 */
export function generateCustomSuggestionsForStaff(job, staffMember, schedule, maxSuggestions = 5, daysToCheck = 10) {
