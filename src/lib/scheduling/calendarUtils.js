 ...
 */
export function generateTimeBlocks(date, staff, events, includeAvailability = true) {
    const blocks = [];
    ...
    if (includeAvailability) {
        ...
        staff.forEach(staffMember => {
            const isAvailableToday = staffMember.availability[dayOfWeek];
            if (isAvailableToday) {
                ...
                if (staffEvents.length === 0) {
                    ...
                    const sortedEvents = [...staffEvents].sort((a, b) => new Date(a.startTime).getTime() - new Date(b.startTime).getTime());
                ...
                if (staffMember.blockedTimes) {
                    const blockedTimesForDate = staffMember.blockedTimes.filter(bt => bt.date === dateString);
 ...
 */
export function eventsToCalendarFormat(events, jobTitles, staffNames) {
 ...
 */
export function getBusinessHours(staff, date) {
    const dayOfWeek = format(date, "EEEE").toLowerCase();
    // Check if staff is available on this day
    if (!staff.availability[dayOfWeek]) {
        return undefined;
    }
    // Get availability hours for this day
    const availabilityHours = staff.availabilityHours?.[dayOfWeek];
 ...
 */
export function findAvailableSlots(date, duration, staff, events) {
    const results = [];
    // Get day of week
    const dayOfWeek = format(date, "EEEE").toLowerCase();
    ...
    staff.forEach(staffMember => {
        ...
        const availableSlots = generateAvailableTimeSlots(staffMember, date, events.filter(event => event.staffId === staffMember.id), durationMinutes);
 ...
 */
function generateAvailableTimeSlots(staff, date, events, durationMinutes) {
    const dayOfWeek = format(date, "EEEE").toLowerCase();
    const dateString = format(date, "yyyy-MM-dd");
    ...
    const blockedTimes = staff.blockedTimes?.filter(bt => bt.date === dateString) || [];
 ...
 */
function filterBlockedTimes(periodStart, periodEnd, blockedTimes, date) {
