...
interface ScheduleSuggestion {
  ...
  utilizationScore: number; // 0-100, higher means better utilization of staff time
  relevanceScore: number; // 0-100, higher means better skill match
  totalScore: number; // Weighted combination of the above scores
}
...
export function generateSchedulingSuggestions(
  ...
  const eligibleStaff = staff.filter(staffMember => {
    ...
    if (job.jobType === "embroidery") {
      return staffMember.skills.some(skill => 
        skill.toLowerCase().includes("embroidery") || 
        skill.toLowerCase().includes("sewing") ||
        skill.toLowerCase().includes("digitizing")
      );
    } else if (job.jobType === "screen_printing") {
      return staffMember.skills.some(skill => 
        skill.toLowerCase().includes("screen") || 
      ...
      return staffMember.skills.some(skill => 
        skill.toLowerCase().includes("wide format") || 
  ...
  for (const staffMember of eligibleStaff) {
    ...
    for (let dayOffset = 0; dayOffset < daysToCheck; dayOffset++) {
      ...
      const availableSlots = getAvailableTimeSlots(
        ...
        staff,
        existingEvents,
        job.estimatedHours * 60 // Convert hours to minutes
      );
...
function calculateSkillRelevance(staffMember: StaffMember, job: Job): number {
  ...
  const jobTypeKeywords: Record<JobType, string[]> = {
