 ...
 */
function parseJobDataFromText(text, fileName) {
    const data = {};
    ...
    const jobNumberMatch = normalizedText.match(/j(\d{4,5})\b/i) ||
    ...
    const invoiceMatch = normalizedText.match(/invoice\s*#\s*:\s*(\d+)/i) ||
    ...
    const clientMatch = normalizedText.match(/bill\s+to\s*:\s*[#]?\d*\s*([a-z0-9\s&,.]+)/i) ||
    ...
    const descriptionMatch = normalizedText.match(/description\s*:\s*([a-z0-9\s,.]+)(?:\s{2,}|$)/i) ||
    ...
    const wantedByMatch = normalizedText.match(/wanted\s+by\s*:\s*(\d{1,2}\/\d{1,2}\/\d{2,4})/i) ||
    ...
    const totalTimeMatch = normalizedText.match(/total\s+time\s*:?\s*(\d+\.?\d*)/i) ||
 ...
 */
export const parsePdfJobTicket = async (file) => {
