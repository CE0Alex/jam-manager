 ...
 */
function createAndTrackBlobUrl(file) {
 ...
 */
export async function deleteFile(fileUrl, bucket = "job-files") {
    try {
 ...
 */
export async function getSignedUrl(filePath, bucket = "job-files", expiresIn = 3600) {
    try {
        const { data, error } = await supabase.storage
            .from(bucket)
